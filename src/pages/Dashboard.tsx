...
const Dashboard: React.FC = () => {
  ...
  const uniqueCommunes = new Set(forms.map(f => f.participant.address.commune)).size;
  
  // Participants added in the last month
  const lastMonthDate = new Date();
  lastMonthDate.setMonth(lastMonthDate.getMonth() - 1);
  const recentParticipants = forms.filter(f => f.participant.createdAt > lastMonthDate).length;
